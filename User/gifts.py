...
from config import ref_reward, sub_reward, cur_link
...
async def gifts(callback: CallbackQuery, state: FSMContext):
    ...
    builder.add(
        InlineKeyboardButton(
            text="Забрать подарок за реферала", callback_data="RefReward"
        )
    )
    builder.add(
        InlineKeyboardButton(
            text="Забрать подарок за подписку", callback_data="SubscriptionReward"
        )
    )
    builder.adjust(2)
...
async def get_ref_reward(callback: CallbackQuery, state: FSMContext):
    ...
    if not referal:
        await callback.message.answer(
            "У вас отсутствуют рефералы", reply_markup=builder.as_markup()
        )
        return
    if ref_reward.is_digit():
        ...
        await callback.message.answer(f"Вы выиграли мерч! {name} за {cost} БЕСПЛАТНО!")
    else:
        ...
        database.set_money_by_chat_id(
            "+", bd_reward_currency, value, callback.message.chat.id
        )
...
async def check_subscription(message: Message, state: FSMContext):
    ...
    user_channel_status = await bot.get_chat_member(
        chat_id=f"@{link[13:]}", user_id=user_id
    )
    ...
    try:
        if user_channel_status[70] != "left":
            ...
            if sub_reward.is_digit():
                ...
                merch_id, name, cost = data
                database.create_order(merch_id, 0, user_id)
                await message.answer(f"Вы выиграли мерч! {name} за {cost} БЕСПЛАТНО!")
            else:
                value = sub_reward.split(" ")[0]
                reward_currency = sub_reward.replace(f"{value} ", "")
                bd_reward_currency = reward_currency.replace(" ", "_").lower()
                database.set_money_by_chat_id(
                    "+", bd_reward_currency, value, message.chat.id
                )
                await message.answer(f"Вы выиграли {value} {reward_currency}!")
        else:
            builder = InlineKeyboardBuilder()
            builder.row(InlineKeyboardButton(text="Подписаться", url=cur_link[19:]))
            await message.answer("Вы не подписаны", reply_markup=builder.as_markup())
            return
    except:
