...
async def show_quizzes(callback: CallbackQuery, state: FSMContext):
    ...
    if not quizzes:
        await callback.message.answer(
            "Квизы отсутствуют", reply_markup=builder.as_markup()
        )
        return
    for name in quizzes:
        name, id = name
    ...
    await callback.message.answer("Выберите квиз: ", reply_markup=builder.as_markup())
...
async def show_quiz(callback: CallbackQuery, state: FSMContext):
    ...
    if database.get_quiz_passed_by_user_id(user_id, id):
        await callback.message.answer(
            text="Вы уже проходили этот квиз.", reply_markup=builder.as_markup()
        )
        return
