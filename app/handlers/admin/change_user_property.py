...
router = Router(name="admin-change-user-property")
...
async def enter_change_user_value(message: Message, state: FSMContext):
    ...
    builder.row(
        InlineKeyboardButton(text="Подтвердить",
                             callback_data="ConfirmChangeUserValue"))
    match data["change_value"]:
        case "Name":
            first_name, last_name = result
            await message.answer(
                "Вы хотите поменять имя и фамилию пользователя {} {} на {} {}?"
                .format(data["first_name"], data["last_name"], first_name,
                        last_name),
                reply_markup=builder.as_markup(),
            )
        case "Level":
            await message.answer(
                "Вы хотите изменить уровень пользователя {} {} c {} на {}?".
                format(data["last_name"], data["first_name"], data["level"],
                       result),
                reply_markup=builder.as_markup(),
            )
        case "Username":
            await message.answer(
                "Точно нужно изменить никнейм пользователя {} {} с {} на {}?".
                format(data["last_name"], data["first_name"], data["username"],
                       result),
                reply_markup=builder.as_markup(),
            )
...
async def confirm_change_user_value(callback: CallbackQuery,
                                    state: FSMContext):
    data = await state.get_data()
    match data["change_value"]:
        case "Name":
            new_first_name, new_last_name = data["result"]
            database.set_user_name_by_user_id(new_first_name, new_last_name,
                                              data["user_id"])
            await callback.message.answer(
                "Вы успешно сменили имя и фамилию пользователя с {} {} на {} {}."
                .format(data["first_name"], data["last_name"], new_first_name,
                        new_last_name))
        case "Level":
            database.set_user_level_by_user_id(data["result"], data["user_id"])
            await callback.message.answer(
                "Вы успешно изменили уровень пользователя {} {} c {} на {}.".
                format(data["last_name"], data["first_name"], data["level"],
                       data["result"]))
        case "Username":
            database.set_username_by_user_id(data["result"], data["user_id"])
            await callback.message.answer(
                "Вы успешно изменили никнейм пользователя {} {} с {} на {}.".
                format(
                    data["last_name"],
                    data["first_name"],
                    data["username"],
