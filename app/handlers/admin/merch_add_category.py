...
router = Router(name="admin-merch-add-category")
...
async def add_merch_category(callback: CallbackQuery, state: FSMContext):
    ...
    builder.row(
        InlineKeyboardButton(text="Подтвердить",
                             callback_data="AddMerchItemCategoryConfirm"))
    await callback.message.answer("Вы точно хотите добавить новую категорию?",
                                  reply_markup=builder.as_markup())


@router.callback_query(F.data == "AddMerchItemCategoryConfirm")
async def add_merch_category_confirm(callback: CallbackQuery,
                                     state: FSMContext):
    builder = InlineKeyboardBuilder()
    builder.row(
        InlineKeyboardButton(text="Админ панель", callback_data="Admin"))
    builder.row(
        InlineKeyboardButton(
            text="Точно уверен!",
            callback_data="AddMerchItemCategoryConfirmConfirm"))
...
async def add_merch_category_confirm_confirm(callback: CallbackQuery,
                                             state: FSMContext):
    builder = InlineKeyboardBuilder()
    builder.row(
        InlineKeyboardButton(text="Админ панель", callback_data="Admin"))
    ...
    await callback.message.answer("Убрать обратно её будет нельзя!",
                                  reply_markup=builder.as_markup())


@router.callback_query(F.data == "AddMerchCategoryItemConfirmConfirmConfirm")
async def add_merch_category_confirm_confirm_confirm(callback: CallbackQuery,
                                                     state: FSMContext):
    builder = InlineKeyboardBuilder()
    builder.row(
        InlineKeyboardButton(text="Админ панель", callback_data="Admin"))
    await callback.message.answer("Хорошо, введите название новой категории.",
                                  reply_markup=builder.as_markup())
...
async def add_merch_category_enter(message: Message, state: FSMContext):
    ...
    builder.row(
        InlineKeyboardButton(text="Админ панель", callback_data="Admin"))
    builder.row(
        InlineKeyboardButton(text="Да, это так",
                             callback_data="AddMerchCategory"))
...
async def add_merch_category_enter_confirm(callback: CallbackQuery,
                                           state: FSMContext):
...
async def add_merch_category_total_confirm(callback: CallbackQuery,
                                           state: FSMContext):
    builder = InlineKeyboardBuilder()
    builder.row(
        InlineKeyboardButton(text="Админ панель", callback_data="Admin"))
