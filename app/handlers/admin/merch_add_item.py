...
router = Router(name="admin-merch-add-item")
    ...
    for category in categories:
        builder.add(
            InlineKeyboardButton(
                text=category,
                callback_data=
                f"AddMerchItemCategory_{categories.index(category)}",
            ))
    builder.add(
...
async def add_merch_item_photo(message: Message, state: FSMContext):
    ...
    builder.row(
        InlineKeyboardButton(text="Админ панель", callback_data="Admin"))
    await message.answer("Пришлите фотографию для витрины",
                         reply_markup=builder.as_markup())
...
async def add_merch_item_photo_confirm(message: Message, state: FSMContext,
    ...
    builder.row(
        InlineKeyboardButton(text="Подтвердить",
                             callback_data="AddMerchItemConfirm"))
    data = await state.get_data()
...
async def add_merch_item_confirm(callback: CallbackQuery, state: FSMContext):
    ...
    database.create_merch(data["name"], data["category_number"],
