...
Описание: {description}
    """
    builder.row(
        InlineKeyboardButton(text="Купить", callback_data=f"BuyMerch_{id}"))
    builder.row(
        InlineKeyboardButton(text="⬅", callback_data="ShowMerchChange_0"),
        InlineKeyboardButton(
            text="➡",
            callback_data="ShowMerchChange_{}".format(2 if category_size >
                                                      1 else 0),
        ),
    )
...
Описание: {description}
    ...
    builder.row(
        InlineKeyboardButton(
            text="⬅",
            callback_data=
            f"ShowMerchChange_{good_num-1 if good_num != 1 else 0}",
        ),
        InlineKeyboardButton(
            text="➡",
            callback_data=
            f"ShowMerchChange_{2 if category_size > good_num else 0}",
        ),
    )
...
async def buy_merch(callback: CallbackQuery, state: FSMContext):
    ...
    builder.row(
        InlineKeyboardButton(text="Подтвердить",
                             callback_data="BuyMerchConfirm_%s" % merch_id))
    await callback.message.answer(
