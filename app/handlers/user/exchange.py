...
from app.config import convert_value, course, currencies

import re

router = Router(name="user-exchange")
...
async def exchange_money(callback: CallbackQuery, state: FSMContext):
    ...
    builder.row(
        InlineKeyboardButton(
            text="Обменять Lucky на E Coin",
            callback_data="Exchange-LuckyToAnother_Value",
        ))
    builder.row(
        InlineKeyboardButton(
            text="Обменять E Coin на Lucky",
            callback_data="Exchange-Another_ValueToLucky",
        ))
    builder.row(
        InlineKeyboardButton(
            text="Обменять Cash Online на E Coin",
            callback_data="Exchange-Cash_OnlineToAnother_Value",
        ))
    builder.row(
        InlineKeyboardButton(
            text="Обменять E Coin на Cash Online",
            callback_data="Exchange-Another_ValueToCash_Online",
        ))
    await callback.message.answer("Какую валюту на какую вы хотиsте обменять?",
...
async def exchange_value_to_value(callback: CallbackQuery, state: FSMContext):
    builder = InlineKeyboardBuilder()
    builder.add(
        InlineKeyboardButton(text="Отменить", callback_data="ExchangeMoney"))
    currencyes = callback.data.split("-")[1].split("To")
    availables = database.get_two_money_values_by_chat_id(
...
async def enter_exchange_value_to_value(message: Message, state: FSMContext):
    builder = InlineKeyboardBuilder()
    builder.row(
        InlineKeyboardButton(text="Вернуться", callback_data="ExchangeMoney"))
    ...
    await state.clear()
    needed = (convert_value(result, data["second"], data["first"])
              if data["transfer"] == "to" else result)
...
""",
    ...
    builder.row(
        InlineKeyboardButton(text="Подтвердить",
                             callback_data="ConfirmExchangeValue"))
    ...
    await message.answer(
        text=
        f"""Вы хотите обменять {needed} {data["first"]} на {out} {data["second"]}?""",
        reply_markup=builder.as_markup(),
    )


@router.callback_query(F.data == "ConfirmExchangeValue")
async def confirm_exchange_value_to_value(callback: CallbackQuery,
    ...
    datum = database.get_money_user_id_by_chat_id(
        data["needed_currency"].lower().replace(" ", "_"),
        callback.message.chat.id)
...
У вас {available} {data["needed_currency"]}""",
        ...
        return
    database.set_money_by_chat_id("-", data["needed_currency"], data["needed"],
                                  callback.message.chat.id)
    database.set_money_by_chat_id("+", data["out_currency"], data["out"],
                                  callback.message.chat.id)
