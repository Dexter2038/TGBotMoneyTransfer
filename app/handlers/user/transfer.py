...
async def give(callback: CallbackQuery, state: FSMContext):
    ...
    builder.row(
        InlineKeyboardButton(text="Cash Online",
                             callback_data="Give-Cash_Online"))
    builder.row(
        InlineKeyboardButton(text="E Coin",
                             callback_data="Give-Another_Value"))
    await callback.message.answer("Какую валюту Вы хотите передать?",
...
async def give_currency(callback: CallbackQuery, state: FSMContext):
    ...
    available = database.get_money_by_chat_id(currency.lower(),
...
Введите имя пользователя.""",
    ...
    builder.row(
        InlineKeyboardButton(text="Подтвердить",
                             callback_data="ConfirmGiveMoney"))
    await message.answer(
        f"""
...
async def give_money_to_person_confirm(callback: CallbackQuery,
                                       state: FSMContext, bot: Bot):
    builder = InlineKeyboardBuilder()
    builder.add(
        InlineKeyboardButton(text="Вернуться", callback_data="GiveMoney"))
    data = await state.get_data()
    datum = database.get_money_user_id_by_chat_id(data["currency"].lower(),
                                                  callback.message.chat.id)
...
У Вас {available} {data["currency]}.""",
        ...
        return
    database.set_money_by_chat_id("-", data["currency"].lower(),
                                  data["amount"], callback.message.chat.id)
    database.set_money_by_username("+", data["currency"].lower(),
