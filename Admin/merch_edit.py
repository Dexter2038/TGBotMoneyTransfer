...
async def merch_change_property_confirm(callback: CallbackQuery, state: FSMContext):
    ...
    match data["property"]:
        ...
        case "Category":
            ...
            database.set_merch_parameter_value_by_id("category", category, data["id"])
            await callback.message.answer(
                f"Категория успешно изменена на {categories[int(category)]}",
                reply_markup=builder.as_markup(),
            )
        case "Cost":
            database.set_merch_parameter_value_by_id("cost", data["value"], data["id"])
            await callback.message.answer(
                "Цена успешно изменена на {}".format(data["value"]),
                reply_markup=builder.as_markup(),
            )
        case "Name":
            database.set_merch_parameter_value_by_id("name", data["value"], data["id"])
            await callback.message.answer(
                "Название успешно изменено на {}".format(data["value"]),
                reply_markup=builder.as_markup(),
            )
        case "Description":
            database.set_merch_parameter_value_by_id(
                "description", data["value"], data["id"]
            )
            await callback.message.answer(
