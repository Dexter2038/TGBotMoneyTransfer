...
async def scan_qr(message: Message, state: FSMContext):
    ...
    match data["type"]:
        case "withdraw":
            status = database.get_withdraw_status_by_amount_and_id(data["qr_id"])
            if status == 1:
...
Клиент собирается вывести %s Cash Online."""
            ...
            database.create_transaction_default(2, data["user_id"], data["amount"], 1)
            await message.answer(
                """
        ...
        case "transactions":
            ...
            builder.row(
                InlineKeyboardButton(
                    ...
                    callback_data="SeeTransactionsQR_0_{}".format(data["user_id"]),
                )
            )
            await message.answer(
                "Просмотреть историю транзакций?", reply_markup=builder.as_markup()
            )
