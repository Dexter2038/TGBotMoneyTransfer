...
async def start_mailing(callback: CallbackQuery, state: FSMContext):
    ...
    builder.row(
        InlineKeyboardButton(text="Реферальная ссылка", callback_data="MailingRef")
    )
    builder.row(
        InlineKeyboardButton(text="Подписка на канал", callback_data="MailingLink")
    )
    builder.row(
        InlineKeyboardButton(
            text="Выставить награду за реферала", callback_data="SetRefReward"
        )
    )
    builder.row(
        InlineKeyboardButton(
    ...
    await callback.message.answer(
        "Выберите способ рассылки", reply_markup=builder.as_markup()
    )
...
async def start_link_mailing(callback: CallbackQuery, state: FSMContext):
    ...
    builder.row(InlineKeyboardButton(text="Админ панель", callback_data="Admin"))
    await callback.message.answer(
        text="Введите ссылку, по которой нужно войти", reply_markup=builder.as_markup()
    )
...
async def enter_link_mailing(message: Message, state: FSMContext):
    ...
    builder.row(InlineKeyboardButton(text="Админ панель", callback_data="Admin"))
    builder.row(
        InlineKeyboardButton(text="Подтвердить", callback_data=f"MailingLinkConfirm_")
    )
...
async def confirm_link_mailing(callback: CallbackQuery, state: FSMContext):
    ...
    database.set_no_sunscription_to_all()
    for chat_id in chat_ids:
        await bot.send_message(
            int(chat_id[0]),
